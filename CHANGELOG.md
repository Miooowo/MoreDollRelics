...
---
## [v0.7-fix2 2026-07-09]

### 移除 (Removed)
- 移除遗物「玩偶的邀请函」及其全部逻辑代码（遗物模型、涅奥候选注入、第二幕问号房强制补丁、图鉴涅奥先古挂载补丁）。
- 移除邀请函相关中英文遗物与事件文案键。
...
### 修复 (Fixed)
- 修复「玩偶的邀请函」在涅奥选项与图鉴加载时可能触发 `RelicModel.Pool` 异常导致崩溃的问题。
- 修复旧版 `DollRoomSpawnPatch` / `Act1EliteCounterPatch` 在部分游戏版本找不到目标方法时导致模组初始化失败的问题。
- 修复图鉴先古分页中「涅奥先古遗物」未稳定显示邀请函的问题。

### 变更 (Changed)
- 邀请函加入 `EventRelicPool` 以兼容动态描述与悬浮提示的池查询逻辑。
- 涅奥邀请函选项改为安全 HoverTip 构造流程，避免依赖会触发池查询的路径。

---
## [v0.7 2026-07-09]

### 新增 (Added)
- 新增涅奥先古遗物 **玩偶的邀请函**：击败 3 名精英后，第二幕满足条件的问号房可触发玩偶室，触发后失效。
...
- **狗王玩偶：**
遗物效果增加：
每3场战斗获得1点生命上限，在敌人回合内受到一次不低于生命值上限20%的伤害时，击晕敌人并在下回合开始获得3层无实体。
- **王筱巫玩偶：**
生命回复提高到7点，复活时打出所有卡牌。

#### 削弱
**娜娜因猫球：**
每花费1能量获得4格挡→每花费1能量获得2格挡
